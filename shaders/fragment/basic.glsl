...
void main(void) {
	...
	float maxBarDepth = 0.95;
	if (gl_FragCoord.z < maxBarDepth) {
		float m = 1.0 / 512.0;
		...
		float farness = pow(gl_FragCoord.z / maxBarDepth, 10.0);
		float barWidth = 0.05 + (0.1 - 0.05) * farness;
		if (k < barWidth) {
			adjust -= 0.3 * (1.0 - farness) * (1.0 - k / barWidth);
		}
	}
	gl_FragColor = vec4(
		color.r + adjust,
